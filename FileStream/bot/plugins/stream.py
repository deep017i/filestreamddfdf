...
async def channel_receive_handler(bot: Client, message: Message):
    ...
    try:
        ...
        # Create a deep link with channel ID and message ID
        channel_id = str(message.chat.id).replace("-100", "")
        message_id = str(message.id)
        deep_link = f"https://t.me/{FileStream.username}?start=channel_{channel_id}_{message_id}_{inserted_id}"
        
        await bot.edit_message_reply_markup(
            chat_id=message.chat.id,
            message_id=message.id,
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("Dᴏᴡɴʟᴏᴀᴅ ʟɪɴᴋ 📥", url=deep_link)]])
        )
